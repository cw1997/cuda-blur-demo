...
// CUDA blur entry point
// Usage: blur_cuda -i <input.bmp> [-u <output.bmp>] [-r <radius>]...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "-i" && i + 1 < argc) {
            ...
            std::cerr << "Usage: blur_cuda -i <input.bmp> [-u <output.bmp>] [-r <radius>]...\n";
            return 1;
        }
    }
    ...
    // Run CUDA blur and measure execution time
    double ms = blurImageGPU(image, blurred, w, h, kernelSize);
